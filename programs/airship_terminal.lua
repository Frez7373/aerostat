...
local function loadConfig()
    ...
    if type(data) == "table" then
        ...
        REDSTONE_SIDE = data.redstoneSide or REDSTONE_SIDE
    end
end
...
local function sensors()
    local ok, v = pcall(altitudeSensor.getHeight)
    if ok and type(v) == "number" then currentHeight = v end
    ok, v = pcall(altitudeSensor.getVerticalSpeed)
...
local function controlHeight()
    ...
    if error > 0 then
        if verticalSpeed > MAX_ASCEND_SPEED then
            setBurner(burnerSignal - 1)
        elseif verticalSpeed < MAX_ASCEND_SPEED * 0.55 then
            setBurner(burnerSignal + 1)
        end
    else
        if verticalSpeed < -MAX_DESCEND_SPEED then
            setBurner(burnerSignal + 1)
        elseif verticalSpeed > -MAX_DESCEND_SPEED * 0.45 then
            setBurner(burnerSignal - 1)
        end
        if mode == "LAND" and math.abs(error) < 5 and verticalSpeed < -0.35 then
            setBurner(burnerSignal + 1)
        end
    end
end
    ...
    while running do
        ...
        if senderId and type(msg) == "table" and msg.auth == AUTH_CODE then
            ...
            elseif msg.type == "SET_HEIGHT" then
                ...
                if h then targetHeight = h; mode = "HOLD"; rednet.send(senderId, status(), PROTOCOL) end
            ...
            elseif msg.type == "SET_CONFIG" then
                if tonumber(msg.terminalId) then TERMINAL_ID = tonumber(msg.terminalId) end
                if type(msg.redstoneSide) == "string" and redstone.getSides()[msg.redstoneSide] then REDSTONE_SIDE = msg.redstoneSide end
